...
{var defaults={fadeDuration:250,zIndex:(typeof modal_zindex!=='undefined'?modal_zindex:9999)};var buttonsText='',title='';for(var i in options.buttons)
{buttonsText+=templates.modal_button.replace('__title__',options.buttons[i].title);}
if($.isArray(message)){title=message[0];message=message[1];}else{title=lang.confirm_title;}
var html=templates.modal.replace('__buttons__',buttonsText).replace('__message__',message).replace('__title__',title);var modal=$(html);modal.modal($.extend(defaults,options));var buttons=modal.find('.modal_buttons > .button');buttons.on('click',function(e)
...
{var expander=$(this);if(!expander||expander.attr("id")==false)
{return;}
expander.on('click',function()
{expandables.expandCollapse($(this));});expander.css("cursor",MyBB.browser=="ie"?"hand":"pointer");});}},expandCollapse:function(element)
{var controls=element.attr("id").replace("_img",""),expandedItem=$("#"+controls+"_e");if(expandedItem.length)
{var expState=+!expandedItem.is(":hidden"),expcolImg=element.attr("src"),expText=[lang.expcol_collapse,lang.expcol_expand];expandedItem.toggle("fast",this.expCallback(controls,expState));element.attr({"alt":expText[expState],"title":expText[expState],"src":expState?expcolImg.replace('collapse.','collapse_collapsed.'):expcolImg.replace('collapse_collapsed.','collapse.')}).parents(':eq(1)').toggleClass(element.parents(':eq(1)').hasClass('thead')?'thead_collapsed':'tcat_collapse_collapsed');this.saveCollapsed(controls,expState);}
...
Cookie.set('collapsed',newCollapsed.join("|"));},expCallback:function(id,state)
{}};var lang={};(function($){if(typeof $.modal!='undefined')
